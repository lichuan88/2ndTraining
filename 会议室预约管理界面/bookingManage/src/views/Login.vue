...
import { addUser } from '@/api/methods/login'
...
const { send: submit,onSuccess:getToken,onError:logerr } = useRequest((user: User) => addUser(user), { immediate: false })
...
getToken((res)=>{
store.access_token=(res.data.data as any).token
})
logerr(()=>{
...
const submitForm = async () => {
    ...
    if (valid) {
        submit(user).then(() => {
            ...
            setTimeout(() => {
                router.push({ name: 'default', path: '/default' })
            }, 1500);
        })
    } else {
        snackbarError.value = true
...
<template>
        ...
        <div class="main">
            ...
            <div class="container">
                <v-form ref="formRef" :model="user">
                    ...
                    <v-text-field v-model="user.account" label="账号" variant="outlined" required :rules="Rules.account"
                    ...
                    <v-btn color="blue-lighten-1 rounded" width="120" style="margin: 0 38%;" @click="submitForm">登录</v-btn>
                </v-form>
            </div>
        </div>
    </div>
    ...
    <v-snackbar :timeout="1500" color="#f0f9eb" class="topSnackbar" v-model="snackbarSuccess">
        <div class="fontS"><v-icon start icon="mdi-checkbox-marked-circle"></v-icon>登录成功，即将跳转</div>
    </v-snackbar>
